	...
	int inferHeight)
{
	...
	logfile_mode1 << "Loading model takes:" << loading_time << "ms\n";
}
	...
	unsigned char* inferdata, int inwidth, int inheight, unsigned char* out, bool debug_flag)
{
	...
	if (frame_count == 100)
	{
		logfile_mode1 << "Style transfer takes " << total_inference_time / frame_count << "ms\n";
	...
	int inferHeight)
{
	...
	std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
	loading_time = static_cast<double>(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count());
	logfile_mode2 << "Loading model takes:" << loading_time << "ms\n";
}
	...
	bool debug_flag)
{
	...
	if (frame_count == 100)
	{
		logfile_mode2 << "Style transfer takes " << total_inference_time / frame_count << "ms\n";
		total_inference_time = 0.0;
		frame_count = 0;
	}
